...
import { Search, X, ArrowRight, ArrowLeft, CornerDownLeft, Sparkles, Building2, ShieldCheck, FileText, Layers, Briefcase } from 'lucide-react';
...
export default function GlobalSearch({ locale, variant = 'header', className = '' }: GlobalSearchProps) {
  ...
  // Keyboard shortcut (Cmd+K / Ctrl+K) to open search anywhere
  useEffect(() => {
    ...
    <>
      ...
      {variant === 'header' ? (
        <button
          ...
          className={`inline-flex items-center gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-all text-xs font-medium cursor-pointer shrink-0 ${className}`}
          aria-label={isRtl ? 'بحث في الموقع (Ctrl+K)' : 'Search site (Cmd+K)'}
          title={isRtl ? 'بحث في الموقع (Ctrl+K)' : 'Search site (Cmd+K)'}
        >
        ...
        <button
          ...
          type="button"
          onClick={() => setIsOpen(true)}
          className={`w-full flex items-center justify-between p-2.5 rounded-lg bg-zinc-100 dark:bg-zinc-800/80 text-zinc-700 dark:text-zinc-200 text-xs font-medium transition-colors ${className}`}
          aria-label={isRtl ? 'بحث في الموقع' : 'Search site'}
        >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs">
                    {[
                      ...
                      <button
                        ...
                        className="flex items-center justify-between p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 text-start transition-colors"
                      >
                        <div className="flex items-center gap-2">
